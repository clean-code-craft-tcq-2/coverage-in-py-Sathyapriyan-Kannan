...
class TypewiseAlert:
    ...
    def get_cooling_type_object(self, cooling_type_input):
        return self.__cooling_types.get(cooling_type_input)
    ...
    def get_alert_object(self, breach_type):
        ...
        return self.__breach_alerts.get(breach_type)
    ...
    def check_and_alert(self, alert_target, battery_char, temperature_in_c):
        ...
        alert_target_func = self.__alert_target_funcs.get(alert_target)
    ...
    def send_to_controller(self, breach_alert, temperature_in_c):
        ...
        is_alert_sent = self.send_alert_to_controller(self.__controller_interface, temperature_in_c, breach_alert)
        return is_alert_sent

    def send_to_email(self, breach_alert, temperature_in_c):

        is_email_sent = self.send_alert_to_email(self.__mail_server_interface, temperature_in_c, breach_alert)
    ...
    def print_alert_to_controller(self, breach_alert):
        header = 0xfeed
        print(f'{header}, {list(self.__breach_alerts.keys())[list(self.__breach_alerts.values()).index(breach_alert)]}')
        return True
